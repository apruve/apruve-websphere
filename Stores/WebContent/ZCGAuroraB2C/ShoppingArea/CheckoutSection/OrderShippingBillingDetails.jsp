							...
							<!-- There are two parts in the content (editAddressContents and mainContents Div)..One Div contains the entire checkoutContents (shopping cart, shipping address, billing info and other things.. The second DIV contains only edit Address page .. On click of Edit Address button, the first div will be hidden and edit address page div will be displayed...
								...
								<div class="main_content" style="display:block">
										...
										<div id="box">                                          
											...
											<flow:ifEnabled feature="SharedShippingBillingPage">
												...
												<div class="body shipping_billing_height" id="WC_ShipmentDisplay_div_30">
													...
													<%System.out.println("after calling checkoutpaymentsBillingAddress.jspf"); %>
													<%@ include file="OrderAdditionalDetailExt.jspf"%>
													<%System.out.println("after calling OrderAdditionalDetailExt.jspf"); %>
												</div>
											</flow:ifEnabled>
											...
											<div class="button_footer_line" id="WC_ShipmentDisplay_div_32_1"> 
												...
												<!-- Apruve Requirement - start -->
												<div id="apruveButton" style="display:none;float: right;padding-right: 550px;"><div id="apruveDiv"/></div>
												<!-- Apruve requirement - End -->
									...
									<div id="editAddressContents" style="display:none">
										...
										<script type="text/javascript">
										//----- Apruve Requirement Starts ---------//
											var apruveExists = '';
											dojo.addOnLoad(function() { 
												apruveExists = '${apruvePaymentExists}';
																
												if(apruveExists == 'true')
												{
													CheckoutPayments.updatePaymentObject(1, 'billing_address_id');
												}
												
												parseWidget("editShippingAddressArea1");
											});
											function toggleCheckoutOption(val) 
										    {
										    	
										    	var needToHide = val.value;
										    	if(val.id == 'PP')
										    	{
										    		if(apruveExists == 'true')
										    		{
										    			
														document.getElementById(needToHide).value = "empty";
														CheckoutPayments.loadPaymentSnippet(document.getElementById(needToHide), 1); 
										    			document.getElementById(needToHide).style.display = "none";
										    			//If there exists a Apruve Payment Method then adding a hidden parameter when apruve radio button is checked
										    			document.getElementById('refForPayMethod').innerHTML = '<input type="hidden" name="payMethodId" value="Apruve"/>';
										    		}
										    			
										    		else
										    		{
										    			document.getElementById('apruveButton').style.display = "";
										        		document.getElementById('shippingBillingPageNext').style.display = "none";
														document.getElementById('WC_ShipmentDisplay_div_32_3').style.display = "none";
														document.getElementById(needToHide).value = "empty";
														CheckoutPayments.loadPaymentSnippet(document.getElementById(needToHide), 1); 
										        		document.getElementById(needToHide).style.display = "none";
										        	 }
										        ...
										        {
										         document.getElementById('refForPayMethod').innerHTML = ""
										         document.getElementById('apruveButton').style.display = "none";
										         document.getElementById('shippingBillingPageNext').style.display = "";
												 document.getElementById('WC_ShipmentDisplay_div_32_3').style.display = "";
										         document.getElementById(needToHide).style.display = "";
										        }
										            return false;
										    }	
										    //----- Apruve Requirement End ---------//	
...
<%}catch(Exception e){System.out.println("exception in checkoutPayments is : "+e.getMessage());e.printStackTrace();}%>