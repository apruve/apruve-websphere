...
CheckoutPayments = {
	...
	*/
	populateProtocolData:function(formName,params){
		...
		params = this.updateParamObject(params,"payMethodId",this.getApruveValue(formName,"payMethodId"),true);
		...
		/* Apruve requirement - start*/
		params = this.updateParamObject(params,"apruveId",this.getValue(formName,"apruveId"),true);
		/* Apruve requirement - end*/
		return params;
	...
	populateProtocolDataForGivenFields:function(formName,params,arrayFields){
		for(var i=0; i<arrayFields.length; i++){
			...
			if(field == 'payMethodId')
				params = this.updateParamObject(params,field,this.getApruveValue(formName,field),true);
			else
				// --- Apruve requirement end ---
	...
	*/
	...
	/* Apruve requirement - start*/
	getApruveValue:function(formName,fieldName){
		if (formName[fieldName] != null)
		{
			//Apruve requirement - start
			//this condition occurs when there is both Apruve paymethodId and the Other payments payMethodId, at that time we need to select only PayPal payMethodId
			if(formName[fieldName].length == 2)
				return formName[fieldName][0].value;
				//Apruve requirement - end
			return formName[fieldName].value;
		}
		else return " ";
	...
	*/
	checkValidPaymentInstructions:function(formNamePrefix) {
		for (var i=1; i<=this.numberOfPaymentMethods; i++) {
			...
			if(i == 1)
			{
				if(document.getElementById('PP').checked == false)
				{
